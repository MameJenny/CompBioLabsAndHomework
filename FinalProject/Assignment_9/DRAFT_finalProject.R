...
#x axis font and angle modification  
  ...
  png(filename="Figure4_mean_temperature_byYear.png")
  mean_temperature_plot
  dev.off()
...
# combining all plots for viewability   
  all_plots_combined <- ggarrange(agricultural_area_plot, avg_biomass_grouped_plot, cereal_hectare_plot, mean_temperature_plot + rremove("x.text"), 
            labels = c("A", "B", "C"),
            ncol = 2, nrow = 2)
 # saving image
  png(filename="Figure5_all_trend_plots.png")
  all_plots_combined
  dev.off()
...
# saving correlogram image
  png(filename="Figure6_correlogram.png")
  correlogram
  dev.off()
# y axis for following plot, showing scaled rankings of data points from highest to lowest 
  ex_y <- c(log2(1:correlation_outliers)) 
...
# plot showing years in which top/bottom biomass and cereal production were recorded
  top_bottom_biomass_cereal_plot <- ggplot(lowest_biomass_years,aes()) +
    geom_point(data = lowest_biomass_years,aes(x = Year, y = ex_y, color = Data_Series[1]), size = 3)+
    geom_point(data = highest_biomass_years,aes(x = Year, y = ex_y, color = Data_Series[3]), size = 3)+
    geom_point(data = bottom_cereal_years,aes(x = Year, y = ex_y, color = Data_Series[5]), size = 3)+
    geom_point(data = top_cereal_years,aes(x = Year, y = ex_y, color = Data_Series[6]), size = 3)+
    scale_x_continuous(breaks = round(seq(min(avg_year_Biomass_grouped$Year), 
                                          max(avg_year_Biomass_grouped$Year), 
                                          by = 1),1)) +
    theme(axis.title.y = element_blank(),
          axis.ticks.y = element_blank(), 
          axis.line.y = element_blank(), 
          axis.text.x = element_text(color="#000000", size=9, angle=40))
          
# saving image    
  png(filename="Figure7_biomass_and_cereal_outlier_years.png", width = 800, height = 698)
...
# plot showing years in which top/bottom biomass and agricultural area were recorded
  top_bottom_biomass_area_plot <- ggplot(lowest_biomass_years,aes()) +
    geom_point(data = lowest_biomass_years,aes(x = Year, y = ex_y, color = Data_Series[1]), size = 3)+
    geom_point(data = highest_biomass_years,aes(x = Year, y = ex_y, color = Data_Series[3]), size = 3)+
    geom_point(data = lowest_area_years,aes(x = Year, y = ex_y, color = Data_Series[2]), size = 3)+
    geom_point(data = highest_area_years,aes(x = Year, y = ex_y, color = Data_Series[4]), size = 3)+
    scale_x_continuous(breaks = round(seq(min(avg_year_Biomass_grouped$Year), 
                                          max(avg_year_Biomass_grouped$Year), 
                                          by = 1),1)) +
    theme(axis.title.y = element_blank(),
          ...
          axis.text.x = element_text(color="#000000", size=9, angle=40))
 # saving image
  png(filename="Figure8_biomass_and_area_outlier_years.png", width = 800, height = 698)
...
# plot showing years in which top/bottom biomass and temperature were recorded 
  top_bottom_biomass_temp_plot <- ggplot(lowest_biomass_years,aes()) +
    geom_point(data = lowest_biomass_years,aes(x = Year, y = ex_y, color = Data_Series[1]), size = 3)+
    geom_point(data = highest_biomass_years,aes(x = Year, y = ex_y, color = Data_Series[3]), size = 3)+
    geom_point(data = top_temperature_years,aes(x = Year, y = ex_y, color = Data_Series[7]), size = 3)+
    geom_point(data = bottom_temperature_years,aes(x = Year, y = ex_y, color = Data_Series[8]), size = 3)+
    scale_x_continuous(breaks = round(seq(min(avg_year_Biomass_grouped$Year), 
                                          max(avg_year_Biomass_grouped$Year), 
                                          by = 1),1)) +
    theme(axis.title.y = element_blank(),
...
# making a grouped plot of all above top/bottom variable plots
  all_outlier_plots_combined <- ggarrange(top_bottom_biomass_area_plot, top_bottom_biomass_cereal_plot, top_bottom_biomass_temp_plot, inherit_aes = FALSE)
