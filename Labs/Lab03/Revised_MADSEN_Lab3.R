...
row.names(CombinedRanking) <- EpisodesNumerals
# lab step #15: giving the data frame row names 
row.names(RankingDataFrame) <- EpisodesNumerals
#
# --- Lab Part III
#
...
tempvariable <- SwapValue["II","Lenny"]
SwapValue["II","Lenny"] <- RankingDataFrame["V", "Lenny"]
SwapValue["V", "Lenny"] <- tempvariable
# lab step #24: using an alternate method of retrieving Penny's ranking for movie III
SwapValue["III","Penny"]
# lab step #25: redoing step #23's change using alternate method
AlternateSwapValue <- RankingDataFrame

AlternateSwapValue$Lenny[5] <- RankingDataFrame$Lenny[2]
AlternateSwapValue$Lenny[2] <- RankingDataFrame$Lenny[5]