...
WeeklyChange <- WeeklyAllowance - (2 * PackGumCost) # change in money every week after week 1

for(i in 1:8 ) {
  WeekChange <- WeekChange + WeeklyChange # adding weekly money change to previous week's money
  print(WeekChange)
  } # calculates change in balance across an 8 week period, using above weekly allowance and cost of gum
...
n[1] <- 2500 # assigning a value of 2500 to n = 1 (initial population abundance)
t <- 1:12 # time
...
AnotherEmpty18[1] <- 1 # assigning first number as 1
...
Fibonacci[0] <- 1 # assigning first two Fibonacci numbers
Fibonacci[1] <- 1
...
print(Fibonacci) # note that this version of the Fibonacci sequence starts with 1,1 as opposed to 0,1
